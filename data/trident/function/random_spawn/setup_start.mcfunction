...
execute if score no_randomtp server matches ..0 run title @a subtitle [{"translate": "trident.title.loading_terrain", "with": [{"selector": "@s"}]}]
execute if score no_randomtp server matches 1.. run title @a subtitle [{"translate": "trident.title.loading_terrain.more", "with": [{"selector": "@s"}, {"score": {"name": "no_randomtp", "objective": "server"}}]}]
